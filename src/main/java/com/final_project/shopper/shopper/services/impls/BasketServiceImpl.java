...
public class BasketServiceImpl implements BasketService {
    ...
    public boolean addToCart(AddToCart addToCart, String email) {
        try {
            ...
            String selectedSize = addToCart.getSize();
            Sizes productSize = findProduct.getSizeQuantities().stream()
                    .filter(s -> s.getSize().equalsIgnoreCase(selectedSize))
                    .findFirst()
                    .orElseThrow(() -> new RuntimeException("This size is not available."));

            Integer quantity = addToCart.getQuantity() == null ? 1 : addToCart.getQuantity();


            if (productSize.getQuantity() < quantity) {
                throw new RuntimeException("There isn’t enough stock in this size.");
            }


            Basket findBasketProduct = basketRepository
                    .findByProductIdAndUserIdAndSize(addToCart.getProductId(), findUser.getId(), selectedSize);
            ...
            if (findBasketProduct == null) {
                ...
                basket.setSize(selectedSize);
                basketRepository.save(basket);
            } else {
                Integer totalQuantity = findBasketProduct.getQuantity() + quantity;
                if (productSize.getQuantity() < totalQuantity) {
