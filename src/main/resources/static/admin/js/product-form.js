...
$(document).ready(function () {
    $('#add-size-btn').off('click').on('click', function () {
        const sizeIndex = $('#sizes-container .size-item').length;

        const newSizeHtml = `
            <div class="size-item">
                <input type="hidden" name="sizesDtos[${sizeIndex}].id" value="">
                <input type="text" name="sizesDtos[${sizeIndex}].size" placeholder="Size name"
                       class="form-control" style="width: 30%;" required>
                <input type="number" name="sizesDtos[${sizeIndex}].quantity" placeholder="Quantity"
